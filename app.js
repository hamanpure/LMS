...
// Edit Course route
app.get("/editcourse", connectEnsureLogin.ensureLoggedIn(), async (req, res) => {
...
// Add a new chapter
app.post('/addchapter', connectEnsureLogin.ensureLoggedIn(), async (req, res) => {
  try {
    ...
    if (req.xhr || req.headers.accept.indexOf('json') > -1) {
      ...
      res.status(500).redirect(`/editcourse/${req.body.courseId}?error=Unable to add chapter`);
    }
  }
});
...
app.post('/addpage', connectEnsureLogin.ensureLoggedIn(), async (req, res) => {
  try {
    ...
    if (req.xhr || req.headers.accept.indexOf('json') > -1) {
      ...
      res.redirect(`/editcourse?course_id=${req.body.courseId}&error=Unable to add page`);
    }
  }
});
...
// Delete a chapter
app.delete('/deletechapter/:id', connectEnsureLogin.ensureLoggedIn(), async (req, res) => {
...
// Delete a page
app.delete('/deletepage/:id', connectEnsureLogin.ensureLoggedIn(), async (req, res) => {
...
//delete course
app.delete('/deletecourse/:id', connectEnsureLogin.ensureLoggedIn(), async (req, res) => {
...
//enorment route
app.post('/enrollment', connectEnsureLogin.ensureLoggedIn(), async (req, res) => {
...
app.post("/mark-complete/:pageId", connectEnsureLogin.ensureLoggedIn(), async (req, res) => {
  try {
...
app.get("/continuecourse/:courseId", connectEnsureLogin.ensureLoggedIn(), async (req, res) => {
  try {
...
app.post("/mark-complete/:id", connectEnsureLogin.ensureLoggedIn(), async (req, res) => {
  try {
...
app.get('/viewcourse/:course_id', connectEnsureLogin.ensureLoggedIn(), async (req, res) => {
  ...
  try {
    ...
    if (course) {
      ...
      res.render('viewcourse', { course, chapters: course.Chapters, csrfToken: req.csrfToken(),educator: educator.dataValues.name });
    } else {
...
app.post("/update-user", connectEnsureLogin.ensureLoggedIn(), async (req, res) => {
  const { fullName, email, oldPassword, newPassword, confirmNewPassword } = req.body;
...
//delete user 
app.delete("/delete-user", connectEnsureLogin.ensureLoggedIn(), async (req, res) => {
  try {
    ...
    if (result) {
      ...
      req.logout((err) => {
        ...
        req.session.destroy((err) => {
          if (err) {
            return res.status(500).json({ error: "Failed to destroy session after deletion" });
          }
          return res.json({ success: true });
        });
      });
